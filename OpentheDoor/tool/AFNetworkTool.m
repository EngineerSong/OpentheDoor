...
+ (void)postUploadWithUrl:(NSString *)urlStr fileUrl:(NSURL *)fileURL success:(void (^)(id responseObject))success fail:(void (^)())fail
{
    ...
    [manager POST:urlStr parameters:nil constructingBodyWithBlock:^(id<AFMultipartFormData> formData) {
        ...
        // 将本地的文件上传至服务器
//        NSURL *fileURL = [[NSBundle mainBundle] URLForResource:@"头像1.png" withExtension:nil];
//    [formData appendPartWithFileURL:fileURL name:@"audio" fileName:[NSString stringWithFormat:@"%@/88.amr",_documentPath] mimeType:@"audio/amr" error:nil];
        [formData appendPartWithFileURL:fileURL name:@"audio" error:NULL];
    } success:^(AFHTTPRequestOperation *operation, id responseObject) {
